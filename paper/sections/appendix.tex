...
This asymmetry suggests that original quality $Q$ and formality $S$ are partially confounded---formally written texts may be inherently perceived as higher quality.
...
\bottomrule
\multicolumn{3}{l}{\footnotesize $^a$ $n = 355$; other rows: $n = 88$.}
\end{tabular}
\end{table}
...
The data support attenuation: $d_{\text{median}} = {-}0.017$ across all sample sizes, with CIs narrowing from ${\pm}0.3$ to ${\pm}0.1$ but never excluding zero.
The same data yield BT $= 66.5\%$ ($p < 0.0001$), confirming that the pairwise signal is robust.
...
\centering
\small
\caption{Bootstrap attenuation analysis: formality $\times$ Qwen3-32B. Likert $d$ remains flat near zero across sample sizes while BT is stable at 66.5\%.}
...
Table~\ref{tab:verb_n355} reports verbosity results at $n = 330$ (from 355 generated pairs, 330 passing NLI gate at $\tau = 0.90$), replicating across all five judges.
...
However, three judges maintain the Blindspot: GPT-4o ($d = {-}.026$, NS), Gemini-2.5-Flash ($d = .087$, borderline), and Claude Sonnet~4 ($d = .032$, NS).
...
GEE Style OR shrinks substantially from the $n = 86$ estimates (Qwen3: $13.10 \to 3.83$; Llama: $8.21 \to 4.01$), consistent with small-sample upward bias at $n = 86$.

\begin{table}[h]
\centering
\small
\caption{Verbosity scaled replication ($n = 330$). Blindspot partially resolves for strong effects (Qwen3, Llama-70B) but persists for weaker-effect judges (GPT-4o, Claude).}
\label{tab:verb_n355}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
GPT-4o & .590$^{***}$ & [.544, .635] & $-$.026 (.813) & $\dagger$ \\
Gemini-2.5 & .678$^{***}$ & [.638, .717] & .087 (.021) & $\dagger$ \\
Claude S4 & .608$^{***}$ & [.567, .649] & .032 (.317) & $\dagger$ \\
\bottomrule
\multicolumn{5}{p{0.95\columnwidth}}{\footnotesize $^{***}$\,$p < 0.001$. $\dagger$ marks Blindspot cases (BT significant, Likert $d$ non-significant at $\alpha = 0.05$ by Wilcoxon). GPT-4o column uses GPT-5.4 as the API-judge replacement. Likert $p$ values from Wilcoxon signed-rank test on non-zero differences.}
\end{tabular}
\end{table}
...
Judges differ markedly in effective granularity usage: GPT-4o clusters scores into ${\sim}5$ distinct bins, Qwen3-32B uses ${\sim}20$, and Llama-70B exploits $50{+}$---yet none resolves the Blindspot.
...
Low-sensitivity reference: Claude Sonnet 4 (\texttt{claude-sonnet-4-20250514}, accessed May 2026).
...
Bradley-Terry parameters are estimated via spectral initialization followed by minorization-maximization \citep{hunter2004mm} using the \texttt{choix} Python library (default regularization).
...
All code, prompts, and experimental data will be released upon acceptance under an open-source license.
...
Sobel mediation test confirms that sentence length mediates 60\% of the total effect ($z = 3.60$, $p < .001$); the 40\% residual direct effect reflects multi-syllable vocabulary shifts (e.g., ``use'' $\to$ ``utilize'') that are inherent to verbose style rather than formality contamination.
...
Table~\ref{tab:cross_bench} reports cross-benchmark replication results on AlpacaEval and MT-Bench for selected configurations targeting the strongest effects from the primary LLMBar analysis.
...
AEval & GPT-4o & Form.\ & .727$^{***}$ & .005 (.909) & $\dagger$ \\
 ...
 & Llama-70B & Form.\ & .756$^{***}$ & 1.120 (${<}.001$) & $\ddagger$ \\
\midrule
MTB & GPT-4o & Form.\ & .692$^{***}$ & .167 (.016) & $\ddagger$ \\
...
AEval & Verb.\ & .683$^{***}$ & $-$.243 (.005) & \textbf{4.76} (${<}.001$) & 0.87 (.732) \\
\midrule
MTB & Form.\ & .733$^{***}$ & .245 (${<}.001$) & 1.64 (.364) & 11.85 (${<}.001$) \\
\bottomrule

"""Claude Sonnet 4 judge: pairwise + Likert scoring + BT/Likert/GEE analysis for formality & verbosity."""
...
async def pairwise_score_record(client, rec, axis, semaphore, limiter):
    async with semaphore:
        ...
        t1_pref = (t1_choice == "A") if t1_choice else None  # True = prefers pole_a
        t2_pref = (t2_choice == "B") if t2_choice else None  # True = prefers pole_a
...
def bt_pair_bootstrap(records, pref_key, n_bootstrap=N_BOOTSTRAP, seed=SEED):
    ...
    return {"n_pairs": n_pairs, "p_pole_a": p_pole_a,
...
def likert_analysis(scored_records, axis):
    ...
    return {"n": len(valid), "ate": ate, "cohen_d": cohen_d,
            "wilcoxon_p": wilcoxon_p, "tie_rate": tie_rate}
...
def gee_analysis(pairwise_records, axis):
    ...
    def _or_ci(name):
        ...
        se = result.bse[name]
        return np.exp(c), result.pvalues[name]

    s_or, s_p = _or_ci("is_style_in_A")
    o_or, o_p = _or_ci("is_original_in_A")

    return {"style_or": s_or, "style_p": s_p,
            "orig_or": o_or, "orig_p": o_p}
...
def run_analysis(pairwise_path, scored_path, axis):
    ...
    blindspot = bt["p_pole_a"] > 0.5 and bt["p_val"] < 0.05

    def fp(p):
        return "< 0.0001" if p < 0.0001 else f"{p:.4f}"

    pole_label = cfg["pole_a"]
    axis_cap = axis.capitalize()
    print(f"\n## Claude Sonnet 4 x {axis_cap} (LLMBar)")
    print(f"Pairs: {bt['n_pairs']}")
    print(f"BT: P({pole_label})={bt['p_pole_a']*100:.1f}% [{bt['ci_lo']*100:.1f}%, {bt['ci_hi']*100:.1f}%], p={fp(bt['p_val'])}")
    ...
    print(f"GEE: Style OR={gee['style_or']:.2f} p={fp(gee['style_p'])}, Orig OR={gee['orig_or']:.2f} p={fp(gee['orig_p'])}")
...
async def main():
    ...
    if args.test_api:
        ...
        try:
            ...
            print(f"API ERROR: {e}")
            alt_models = ["claude-sonnet-4", "anthropic/claude-sonnet-4-20250514"]
            for m in alt_models:
                try:
                    resp = client.chat.completions.create(
                        model=m, messages=[{"role": "user", "content": "Say hello"}], max_tokens=10)
                    print(f"Model '{m}' works: {resp.choices[0].message.content}")
                    break
                except Exception as e2:
                    print(f"Model '{m}' also failed: {e2}")
        return

    axes = [
        {"axis": "formality",
         "input": f"{data_dir}/verified_32b.jsonl",
         "pw_out": f"{data_dir}/pairwise_results_claude_formality.jsonl",
         "sc_out": f"{data_dir}/scored_claude_formality.jsonl"},
        {"axis": "verbosity",
         "input": f"{data_dir}/verified_verbosity.jsonl",
         "pw_out": f"{data_dir}/pairwise_results_claude_verbosity.jsonl",
         "sc_out": f"{data_dir}/scored_claude_verbosity.jsonl"},
    ]
    ...
    for ax in axes:
        ...
        run_analysis(ax["pw_out"], ax["sc_out"], ax["axis"])
